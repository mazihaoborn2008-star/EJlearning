...
const remote=process.argv.includes('--remote'),config=remote?'wrangler.35a.jsonc':'wrangler.35a.local.jsonc',cfg=JSON.parse(fs.readFileSync(config,'utf8'));
assert.equal(cfg.name,'ej-learning-35a');assert.equal(cfg.d1_databases[0].database_name,'ej-learning-35a-db');assert.deepEqual(cfg.routes,[]);
assert(!['e2e8f4c8-27e0-416e-993c-7510259a6b1d','36c72b9b-da1f-425c-9b1a-6603b0f7d150'].includes(cfg.d1_databases[0].database_id));
const location=remote?['--remote']:['--local','--persist-to',process.env.D1_DIRECTORY||'.wrangler/35a-local'];
...
assert.deepEqual(checks[0],[]);assert.equal(checks[1].length,8);assert.equal(checks[2][0].count,183);assert.deepEqual(checks[3],[{language:'en',count:16},{language:'ja',count:17}]);assert.deepEqual(checks[4],[{language:'en',count:9},{language:'ja',count:10}]);assert.equal(checks[5][0].count,12);assert.equal(checks[6][0].count,25);
...
fs.writeFileSync(`docs/phase35a/d1-${remote?'staging':'local'}.json`,JSON.stringify({passed:true,location:remote?'isolated remote D1':location,config,migration_reapplication:'No migrations to apply',checks,negativeChecks:negative.map(([sql,error])=>({sql,expected:error,rejected:true}))},null,2));
console.log('D1 checks passed: clean migration tracking, counts, foreign keys, 11 rejected invalid writes.');
