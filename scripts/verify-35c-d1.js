...
const remote=process.argv.includes('--remote'),config='wrangler.35c.jsonc',cfg=JSON.parse(fs.readFileSync(config));
assert.equal(cfg.name,'ej-learning-35c');assert.equal(cfg.d1_databases[0].database_id,'c854a0e6-d576-4254-b4b6-449fc7517437');
...
assert.deepEqual(await query('PRAGMA foreign_key_check'),[]);const migrations=await query('SELECT name FROM d1_migrations ORDER BY id');assert.deepEqual(migrations.map(x=>x.name),fs.readdirSync('migrations-35c').sort());
const report=audit(data);assert(report.passed,JSON.stringify(report.errors));fs.writeFileSync(`docs/phase35c/audit-${remote?'staging':'d1-local'}.json`,JSON.stringify({...report,verifiedTables:tables.map(x=>x.name),exactMatchToCleanMigration:true,migrations},null,2));console.log('Isolated D1 exact table comparison and curriculum audit passed: '+(remote?'remote':'local'));
