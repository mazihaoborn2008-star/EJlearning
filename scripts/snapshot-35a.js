...
for(const [name,id,base] of [
 ...
 ['ej-learning-phase3','36c72b9b-da1f-425c-9b1a-6603b0f7d150','https://ej-learning-phase3.yanjian-language-learning.workers.dev']]){
...
output.localHashes=Object.fromEntries([...walk('public'),...walk('migrations'),...['src/worker.js','src/assessments.js','src/engine.js','package.json','package-lock.json','wrangler.jsonc','wrangler.phase3.jsonc','wrangler.production.jsonc','wrangler.staging.jsonc']].map(p=>[p,hash(fs.readFileSync(p))]));
const label=process.argv[2]||'before';fs.writeFileSync(`docs/phase35a/integrity-${label}.json`,JSON.stringify(output,null,2));console.log('Protected environments read-only snapshot saved: '+label);
