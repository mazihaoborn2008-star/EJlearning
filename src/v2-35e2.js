...
const definitions={
 vocabulary:{table:'v2_vocabulary_items',fields:'id,language,lemma,type,stage,part_of_speech,register,sort_order,ipa,reading',filters:{language:'language',stage:'stage',type:'type',part_of_speech:'part_of_speech',register:'register'}},
 grammar:{table:'v2_grammar_points',fields:'id,language,slug,title_zh,form_name,level,register,sort_order,purpose_zh',filters:{language:'language',level:'level'}},
...
function params(url,kind,reverse=false){
 const allowed=new Set(reverse?['limit','offset']:['limit','offset','search',...Object.keys(definitions[kind].filters),...(kind==='vocabulary'?['topic','browse']:[]),...(kind==='sentences'?['language','overall_difficulty','vocabulary_difficulty','grammar_difficulty']:[])]);
...
async function list(db,url,kind){
 ...
 if(kind==='vocabulary'&&p.topic){conditions.push('EXISTS (SELECT 1 FROM v2_vocabulary_topics vt WHERE vt.item_id=t.id AND vt.topic_id=?)');args.push(p.topic);}
 if(p.search){
 ...
 const meaning=kind==='vocabulary'?',(SELECT meaning_zh FROM v2_vocabulary_senses s WHERE s.item_id=t.id ORDER BY s.sort_order,s.id LIMIT 1) AS meaning_zh':'';
 ...
 return page(db,`SELECT ${d.fields.split(',').map(f=>'t.'+f).join(',')}${meaning} FROM ${d.table} t`,conditions.join(' AND '),args,p,order);
}
