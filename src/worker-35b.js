...
import {academic} from './academic-35b.js';
export default {
 scheduled:legacy.scheduled,
 async fetch(request,env){
  const url=new URL(request.url);
  if(url.pathname.startsWith('/api/v2/academic'))return academic(request,env.DB);
  if(url.pathname.startsWith('/api/v2/'))return v2(request,env.DB);
  if(url.pathname.startsWith('/api/'))return legacy.fetch(request,env);
  if(url.pathname.startsWith('/legacy/api/')){url.pathname=url.pathname.slice(7);return legacy.fetch(new Request(url,request),env);}
