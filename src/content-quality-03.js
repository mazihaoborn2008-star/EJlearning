...
// Only an occurrence authored inside the selected sentence can control a
// completion. Longest-first order prevents なので/なんですが from being
// reduced to their overlapping shorter endings.
export const controlledCompletionForms=Object.freeze({
  'ja-n-desu-ga':Object.freeze(['なんですが','んですが']),
  'ja-node':Object.freeze(['なので','ので']),
  'ja-honorific':Object.freeze(['おっしゃいました','おっしゃいます','おっしゃった','おっしゃって','おっしゃらない','おっしゃる']),
  'ja-humble':Object.freeze(['いたしました','いたします','いたさない','いたして','いたした','いたす'])
});

export function controlledCompletionAnswer(grammarId,text){
 const forms=controlledCompletionForms[grammarId];
 if(!forms)return null;
 const source=String(text||'');
 for(const form of forms){
  const at=source.indexOf(form);
  if(at>=0&&source.indexOf(form,at+form.length)<0)return form;
 }
 return null;
}
