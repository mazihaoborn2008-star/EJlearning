...
async function rankedVocabulary(db,p,frameworkLanguage,limit,offset){
 const config=(await rows(db,'SELECT * FROM v2_vocabulary_course_targets WHERE framework_id=? AND target=? AND language=?',p.framework,p.target,frameworkLanguage))[0];
 if(!config)return json({error:{message:'词汇课程池尚未建立。'}},404);
 const where=["r.framework_id=?","r.language=?","v.publication_state='published'"],args=[p.framework,frameworkLanguage];
 if(config.pool_mode==='cumulative'){where.push('r.course_rank<=?');args.push(config.pool_size);}else{where.push('r.estimated_target=?');args.push(p.target);}
 if(p.id){where.push('v.id=?');args.push(p.id);}
 if(p.search){const q='%'+p.search.replace(/[\\%_]/g,'\\$&')+'%';where.push("(v.lemma LIKE ? ESCAPE '\\' OR EXISTS (SELECT 1 FROM v2_vocabulary_senses vs WHERE vs.item_id=v.id AND (vs.meaning_zh LIKE ? ESCAPE '\\' OR vs.usage_zh LIKE ? ESCAPE '\\')))" );args.push(q,q,q);}
 ...
 if(p.topic){where.push('EXISTS (SELECT 1 FROM v2_vocabulary_topics vt WHERE vt.item_id=v.id AND vt.topic_id=?)');args.push(p.topic);}
 const order=p.browse&&!p.search?(frameworkLanguage==='ja'?"v.reading,v.lemma,r.course_rank":"lower(v.lemma),r.course_rank"):'r.editorial_boost DESC,r.course_rank,v.id';
 const from='FROM v2_vocabulary_course_ranks r JOIN v2_vocabulary_items v ON v.id=r.item_id';
 const total=(await rows(db,`SELECT count(*) total ${from} WHERE ${where.join(' AND ')}`,...args))[0].total;
 const data=await rows(db,`SELECT (r.framework_id||'-'||coalesce(r.estimated_target,'cumulative')||'-'||v.id) id,r.framework_id,? target,r.language,v.id vocabulary_id,NULL grammar_id,NULL expression_id,CASE WHEN r.editorial_boost=1 THEN '人工审校核心词；同时属于大型课程词池。' ELSE '按课程词频与实用性排序的大型词汇库。' END relevance_zh,CASE WHEN r.editorial_boost=1 THEN '["课程词库","人工推荐"]' ELSE '["课程词库"]' END tags_json,'reviewed' status,r.basis basis_zh,'course_estimate' source_type,CASE WHEN r.framework_id='IELTS' THEN 'JIJI course target' ELSE 'Community/course estimate' END source_title,config.notice_zh editorial_note_zh,v.lemma title,v.id content_id,v.stage,v.ipa,v.reading,v.part_of_speech,v.register vocabulary_register,(SELECT meaning_zh FROM v2_vocabulary_senses vs WHERE vs.item_id=v.id ORDER BY vs.sort_order,vs.id LIMIT 1) meaning_zh,NULL level,NULL form_name,NULL purpose_zh,NULL grammar_register,NULL text,NULL overall_difficulty,NULL context_zh,NULL unit_type,NULL topic_id,'vocabulary' domain ${from} JOIN v2_vocabulary_course_targets config ON config.framework_id=r.framework_id AND config.target=? WHERE ${where.join(' AND ')} ORDER BY ${order} LIMIT ? OFFSET ?`,p.target,p.target,...args,limit+1,offset);
 return json({data:data.slice(0,limit).map(({tags_json,...r})=>({...r,tags:JSON.parse(tags_json)})),pagination:{limit,offset,total,has_more:data.length>limit,next_offset:data.length>limit?offset+limit:null},pool:{mode:config.pool_mode,notice_zh:config.notice_zh}});
}
export async function academic(request,db){
 try{
  ...
  const pos={en:new Set(['adjective','adjective phrase','adverb','auxiliary','conjunction','determiner','fixed expression','interjection','noun','noun phrase','numeral','phrasal verb','preposition','prepositional phrase','pronoun','proper noun','verb','verb phrase']),ja:new Set(['adjectival noun','adjective','adverb','affix','auxiliary','conjunction','counter','fixed expression','interjection','noun','noun phrase','numeral','particle','pronoun','proper noun','verb','verb phrase'])};
  ...
  if(p.domain==='vocabulary'&&p.framework&&p.target)return rankedVocabulary(db,p,frameworkLanguage,limit,offset);
