...
export const posNames={verb:'动词',noun:'名词',adverb:'副词',adjective:'形容词','adjectival noun':'な形容词','verb phrase':'动词短语','phrasal verb':'短语动词','prepositional phrase':'介词短语'};
...
export function shell(active){
 ...
 labelLegacy();new MutationObserver(labelLegacy).observe(document.body,{childList:true,subtree:true});
 $('#header').innerHTML=`<div class="top"><a class="brand" href="/"><span class="brand-mark">言</span>言间</a><span class="muted">中文理解 · 自然表达</span><nav class="nav" aria-label="主要导航">${[['首页','/','home'],['学习','/vocabulary.html','learn'],['学术 / 考试','/academic.html','academic'],['AI 学习','/ai.html','ai'],['我的学习','/progress.html','progress']].map(([n,h,k])=>`<a href="${h}" ${active===k?'aria-current="page"':''}>${n}</a>`).join('')}</nav></div>`;
...
export function alignmentCards(rows){return rows.map(a=>`<div class="alignment">${badge(a.framework_id+' 目标 '+a.target,'exam')} ${badge(a.status==='draft'?'课程参考 · 待审':'已审校')}<p>${esc(a.relevance_zh)}</p><div class="chips">${a.tags.map(t=>badge(t)).join('')}</div><details><summary>分类依据</summary><p>${esc(a.basis_zh)}</p><p>${esc(a.source_title)} · ${esc(a.editorial_note_zh)}</p>${a.last_reviewed_at?`<p>审校时间：${esc(a.last_reviewed_at)}</p>`:''}</details></div>`).join('');}
