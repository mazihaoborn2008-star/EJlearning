...
The migrations use stable IDs plus `INSERT OR IGNORE`. The first creates a separate grammar-example completion-authority table and expression-context version table, adds the approved examples/context versions, and inserts one new bundle row. The second deterministically projects the versioned bundle's eight new lessons, exact current titles/objectives/order, and prerequisite replacements into the current `lesson_units`/`lesson_prerequisites` index required by the existing progress foreign key. Staging validation found that the older index still held the pre-Expansion-01 `s4-l4` parents for the two existing `s5-l1` rows; the forward-only third migration projects the already-correct immutable bundle boundaries to `en-s4-l6` and `ja-s4-l6`. Neither edits `curriculum-expansion-01a-v1`, any historical migration or bundle, learner progress, attempts, SRS state, or settings. Applying them twice leaves one copy of every new example, authority row, context version, lesson, edge, and bundle.

The registered update config points only to `migrations-stage5-expansion-02`; it cannot replay bootstrap, Expansion 01, or Hotfix 03. An explicitly confirmed update may reconcile a newly reviewed pending forward migration even when the dataset marker already matches; the D1 ledger remains the exact-once authority and the policy wrapper rechecks the expected lesson count. Normal Worker deployment remains schema-only and does not apply curriculum migrations.
