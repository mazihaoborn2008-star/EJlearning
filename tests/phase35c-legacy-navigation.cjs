...
try{
 for(const width of [360,390,430,1440]){
  ...
  await p.screenshot({path:'docs/phase35c/'+label+'-'+width+'-v2.png',fullPage:false});
  await p.goto(base+'/legacy/learn.html?return=%2Fsentences.html');await p.locator('#return-v2[data-ready]').waitFor();await p.screenshot({path:'docs/phase35c/'+label+'-'+width+'-legacy.png',fullPage:false});
 ...
 assert.deepEqual(errors,[]);checks.push('Unsafe return targets fall back Home; compatibility labels; JS transition; unavailable storage');fs.writeFileSync('docs/phase35c/'+label+'-navigation.json',JSON.stringify({passed:true,base,checks,errors},null,2));console.log(checks.join('\n'));
