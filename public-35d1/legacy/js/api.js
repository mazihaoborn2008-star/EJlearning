...
async function loadContent(){
 ...
 topics=ts;concepts=cs.map(c=>({...c,enDifficulty:c.en.difficulty,jaDifficulty:c.ja.difficulty}));
}
...
const typeNames={vocabulary:'词语与用法',phrase:'日常短语',sentence:'句子表达',pattern:'表达结构',scenario_response:'情境回应',dialogue:'短对话'};
const detailCache=new Map();
async function loadDetail(id){
 if(detailCache.has(id))return detailCache.get(id);
 const c=await api('/concepts/'+id);
 const detail={...c,enMeta:c.en.metadata||{},jaMeta:c.ja.metadata||{},enRegister:c.en.register,jaRegister:c.ja.register,en:c.en.text,ipa:c.en.ipa,enDifficulty:c.en.difficulty,enChunks:c.en.chunks,enGrammar:c.en.grammar,ja:c.ja.text,hiragana:c.ja.hiragana,jaReadings:c.ja.readings,jaDifficulty:c.ja.difficulty,jaChunks:c.ja.chunks,jaGrammar:c.ja.grammar};
 detailCache.set(id,detail);return detail;
}
